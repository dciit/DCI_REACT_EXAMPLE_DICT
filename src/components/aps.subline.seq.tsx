import { PropApsPlnInfo, PropDataSublineSeq, PropRouter, PropRouterSublineSeq, PropsMain, PropSublineSeq } from '@/interface/aps.interface';
import { ApiAdjStock, APIApsRouter, APIGetApsProductionPlanInfo, APIGetRMInfo, APIGetSublineSeq, APIUpdateApsProdPlan, APIUpdateRMStock, APIUpdateSeqSublineSeq } from '@/service/aps.service';
...
import { EditOutlined, PullRequestOutlined, HolderOutlined, RetweetOutlined, CloseOutlined } from '@ant-design/icons';
...
// interface ParamApsPrdPlanSelected {
//     PrdProdPlan: string;
// }
...
function SublineWIPs(props: ParamMainPlan) {
    ...
    const [SublineLastUpdate, setSublineLastUpdate] = useState<PropSublineLastUpdate[]>([]);
    ...
    const [SublineParam, setSublineParam] = useState<ParamSublineSeq>({ line: '', process: '' })
    ...
    const init = async () => {
        ...
        setRouters(routers);
        if (LineSelected == '') {
            setSublineParam({ line: routers[0].LINE_GROUP, process: routers[0].LINE_CODE })
        }
    }
    useEffect(() => {
        init();
    ...
    const LoadSublineData = async () => {
        setLoad(true);
        // let res = await APIGetWIPSubline(SublineParam.line, SublineParam.process);
    ...
    const TdStatus = (status: string) => {
        ...
        if (status == 'PARTIALY') {
            icon = <BsDiamondHalf />
            style = 'bg-sky-500/10 text-sky-600/90 border border-sky-500/50';
            text = 'ผลิตบางส่วน'
        ...
        try {
            ...
            if (RMIndex != -1) {
                ...
                if (RMInfoOfIndex != null) {
                    let res = await APIUpdateRMStock({ rm_code: RMInfoOfIndex.PART_NO, rm_cm: RMInfoOfIndex.CM, empcode: empcode, stock: Number(RMInfoOfIndex.BAL), rm_wcno: RMInfoOfIndex.WCNO });
                    if (res.status == true) {
                        openNotification('success', `บันทึกข้อมูลเรียบร้อยแล้ว`);
                    } else {
        ...
        <div className='flex flex-col gap-2'>
            ...
            <div className='select-none'>
                <Radio.Group value={SublineParam.line} onChange={(e) => {
                    setSublineParam({ line: e.target.value, process: Routers.find(o => o.LINE_GROUP == e.target.value)?.LINE_CODE as string ?? '' })
                }}>
                    {
                        Array.from(new Set([...Routers].map((o) => o.LINE_GROUP))).map((o, i) => <Radio.Button key={i} value={o}>{o == 'FS' ? 'FS/OS' : o}</Radio.Button>)
                    }
                </Radio.Group>
            </div>
            <div className='flex items-center justify-between select-none'>
                <Radio.Group value={SublineParam.process} onChange={(e) => setSublineParam({ ...SublineParam, process: e.target.value })} >
                    {
                        Array.from(new Set([...Routers.filter(x => x.LINE_GROUP == SublineParam.line).map(x => x.LINE_CODE)])).map((o, i) => <Radio.Button key={i} value={o}>{o == 'FS' ? 'FS/OS' : o}</Radio.Button>)
                    }
                </Radio.Group>
                ...
                <Spin spinning={load}>
                    {
                        ...
                        /> : <div className='overflow-x-auto'>
                            <table id='tbSublineSeq' className='w-fit bg-white table-fixed'>
                                <thead className="sm:text-[10px] md:text-[10px] lg:text-[10px] sticky top-0 bg-[#f9fafb] text-black/75">
                                    <tr>
                                        ...
                                        <th className='border w-[75px]' colSpan={1}>
                                            <div className='flex flex-col'>
                                                <span>RESULT</span>
                                                <span className='tracking-wide text-green-500 font-mono'>(BF)</span>
                                            </div>
                                        </th>
                                        ...
                                        <th className='border w-[50px] py-1 bg-green-800/10'>M/C</th>
                                        {
                                            SublineInfo?.router.map((_, index: number) => {
                                                return <Fragment key={index}>
                                                    <th className="border min-w-[75px] bg-[#6A67F3] text-white">PD</th>
                                                    <th className="border w-[75px] bg-[#498DCA] text-white">PS</th>
                                                </Fragment>
                                            })
                                        }
                                    </tr>
                                ...
                                <tbody className='sm:text-[11px] md:text-[11px] lg:text-[11px] '>

                                    {
                                        SublineInfo?.data.map((item: PropDataSublineSeq, index: number) => {
                                            ...
                                            let models = item.COMMON_SEBANGO.split(',');
                                            let modelSupportPlan = item.COMMON_SEBANGO_SUPPORT_PLAN.split(',');
                                            models.filter(x => !modelSupportPlan.includes(x));
                                            return <Fragment key={index}>
                                                <tr key={index} className={`     ${status == 'Done' && 'bg-green-100/20'} hover:shadow-top-bottom transition-all duration-300 cursor-pointer `}>
                                                    <td className={`bg-[#f9fafb] font-semibold border text-center align-middle py-1`}>{item.PRD_SEQ}</td>
                                                    <td className={`border break-all font-semibold pl-[8px] ${(hasLastUpdate.length > 0 && remain > 0) && 'bg-yellow-300/30'}`}>
                                                        <Popover className='cursor-pointer' content={
                                                            <div className='grid  gap-1'>
                                                                {item.COMMON_SEBANGO}
                                                            </div>
                                                        } title='โมเดลที่ใช้งาน'>
                                                            <span className='tracking-wider text-blue-500'>{item.COMMON_SEBANGO_SUPPORT_PLAN}</span>
                                                        </Popover>
                                                    </td>
                                                    <td className={`border align-top pl-[8px]`}>
                                                        <div className='flex flex-col leading-[1] gap-1 py-1'>
                                                            <strong>{item.COMMON_MODEL}</strong>
                                                            <span>{item.SUBLINE}</span>
                                                            <Paragraph copyable={{ text: item.PART_NO }} style={{ margin: 0, fontWeight: '400', fontSize: '12px' }}>{item.PART_NO} {item.CM != '' ? `(${item.CM})` : ''}</Paragraph>
                                                        </div>
                                                    </td>
                                                    ...
                                                    </td>
                                                    {
                                                        SublineInfo.router.map((oRouter: PropRouterSublineSeq, idxItem: number) => {
                                                            ...
                                                            return <Fragment key={idxItem}>
                                                                <td className={`px-[8px] text-right align-middle pr-1  border font-bold tracking-wide ${StyleOfLine}`} key={idxItem + '-LINE-' + ProcessCode} onClick={() => setSublineRMInfo({ partno: item.PART_NO, process: ProcessCode, type: 'LINE', wcno: item.WCNO })}>
                                                                    {
                                                                ...
                                                                <td className={`px-[8px] text-right align-middle pr-1  border font-bold tracking-wide ${StyleOfPS}`} key={idxItem + '-PS-' + ProcessCode} onClick={() => setSublineRMInfo({ partno: item.PART_NO, process: ProcessCode, type: 'PS', wcno: item.WCNO })}>
                                                                    {
            ...
            <Modal width={800} title={`Raw Material Information in ${SublineRMInfo.type}`} open={openRMDetail} onCancel={() => setOpenRMDetail(false)} onClose={() => setOpenRMDetail(false)} footer={
