...
import { Button, Popconfirm, notification, Spin, Dropdown } from 'antd';
...
import { APIApsGetMainSeq, APIChangeSublineSeq } from '@/service/aps.service';
import { PropMainSeq } from '../interface/aps.interface';
...
function APSMainSeq() {
    ...
    const init = async () => {
        setLoad(true);
        const res = await APIApsGetMainSeq();
        setMainSeq(res.data);
        setLoad(false);
    }
    ...
    return (
        <div className='sm:w-[100%] md:w-[30%] flex flex-col gap-3  bg-gradient-to-r from-green-50 to-teal-50 p-4 border rounded-xl '>
            {contextHolder}
                    ...
                    <tbody className='sm:text-[8px] lg:text-[12px] xl:text-[12px] 2xl:text-[12px]'>
                        {
                            MainSeq.filter(x => x.PLN_STATUS != 'HISTORY').map((o: PropMainSeq, i: number) => {
                                ...
                                // let apsDate = moment(o.YMD).format('YYYYMMDD');
                                        ...
                                        isNextDay && <tr className={`cursor-pointer select-none ${noPlan && 'opacity-50'}`} >
                                            <td colSpan={3} className='border py-2 pl-3'>
                                                <div className='flex w-[100%] items-center gap-2'>
                                                    ...
                                                    {/* <Button disabled={apsDate == moment().subtract(8, 'hour').format('YYYYMMDD') ? false : true} size='small' type={`${(changeSeq.length > 0 && changeSeq == apsDate) ? 'primary' : 'default'}`} icon={<RetweetOutlined />} onClick={() => setChangeSeq(changeSeq != '' ? '' : apsDate)}>สลับลำดับ</Button>
                                                    {
